...
class CustomAuthenticator(Authenticator):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.users_path = '/opt/jupyterhub/users.csv'
        self.allow_existing_users = True  # 根据需要设置
    
    # 新增用户（每次启动JupyterHub都会执行）
    def add_user(self, user):
        username = user.name
        # 本地用户文件中增加用户，但无法配置用户密码，该函数主要目的是在jupyterhub.sqlite中用户数据库增加允许访问Hub的用户
        self._check_user(username, password=username, type_=2)
        
        if not self.validate_username(user.name):
            raise ValueError(f"Invalid username: {user.name}")
            
        if self.allow_existing_users and not self.allow_all:
            # 从hub允许登录集合中添加(jupyterhub.sqlite中用户数据库也会跟增加)
            self.allowed_users.add(user.name)  
    ...
    def delete_user(self, user):
        username = user.name
        # 从本地用户文件中删除
        self._check_user(username, type_=3)  
        # 从hub允许登录集合中删除(jupyterhub,sqlite中用户数据库也会跟着删除)
        self.allowed_users.discard(user.name)  
    ...
    async def authenticate(self, handler, data):
        username = data['username']
        password = data['password']
        if self._check_user(username, password, type_=1):
            return username
        else:
            return None

    def _check_user(self, username, password=None, type_=1):
        # 读取本地用户文件
        users_data = pd.read_csv(self.users_path, dtype={'password': str})
        user_exists = username in users_data['user'].values
        ...
        if type_ == 1:
            ...
            stored_pwd = users_data.loc[users_data['user'] == username, 'password'].iloc[0]
        ...
        # 新增用户 (type_=2)
            ...
            users_data.to_csv(self.users_path, index=False)
            print('用户添加成功')
            return True
        ...
        # 删除用户 (type_=3)
            ...
            # 删除用户并保存
            users_data = users_data[users_data['user'] != username]
            users_data.to_csv(self.users_path, index=False)
            print('用户删除成功')
            return True
